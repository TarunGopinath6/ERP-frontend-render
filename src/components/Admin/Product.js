...
export default function ProductTable() {
  ...
  React.useEffect(() => {
    const getData = async () => {
      ...
      if (get_options)
        setOptions(response?.data?.meta?.options ?? { company: {} });
  ...
  const handleDeleteClick = (uuid) => async () => {
    ...
    const response = await remove("product", uuid);
  ...
  const processRowUpdate = async (newRow) => {
    ...
    console.log(newRow["company_name"], options?.company);
    newRow["company"] = options?.company[newRow["company_name"]];

    const response = await write("product", newRow, isNew);
  ...
  const columns = [
    ...
    },
    {
      ...
      disableColumnMenu: true,
      type: "singleSelect",
      valueOptions: Object.keys(options?.company) ?? [],
    },
    {
