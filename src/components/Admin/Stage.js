...
export default function StageTable() {
  ...
  React.useEffect(() => {
    const getData = async () => {
      ...
      if (get_options)
        setOptions(response?.data?.meta?.options ?? { department: {} });
  ...
  const handleDeleteClick = (uuid) => async () => {
    ...
    const response = await remove("stage", uuid);
  ...
  const processRowUpdate = async (newRow) => {
    ...
    console.log(newRow["department_name"], options?.department);
    newRow["department"] = options?.department[newRow["department_name"]];

    const response = await write("stage", newRow, isNew);
  ...
  const columns = [
    ...
    },
    {
      ...
      disableColumnMenu: true,
      type: "singleSelect",
      valueOptions: Object.keys(options?.department) ?? [],
    },
    {
      field: "index",
      headerName: "Index",
